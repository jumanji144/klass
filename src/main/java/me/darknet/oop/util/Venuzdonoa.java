    ...
    static {
        try {
            ...
            if (os.contains("win")) {
                ...
                if(path.resolve("lib/" + System.getProperty("os.arch") + "/libjava.so").toFile().exists())
                    path = path.resolve("lib/" + System.getProperty("os.arch") + "/libjava.so");
                else path = path.resolve("lib/libjava.so");
            }
            mv.visitCode();
